...
function number2cnChar(num, flag, digitUnit, wordFigure) --flag=0中文小写反之为大写
  ...
  if tonumber(num2) == 0 then
    ...
    elseif lens < 13 then
      ...
      digitUnit[2] .. formatNum(string.sub(num1, -8, -5), flag) .. digitUnit[1] .. formatNum(string.sub(num1, -4, -1),
        flag)
    else
      result = ""
    end
...
function number_translatorFunc(num)
  ...
  if numberPart.dot ~= "" then
    ...
    table.insert(result, { number2cnChar(numberPart.int, 1, { "萬", "億" }, { "〇", "一", "十", "点" }) ..
