...
注：已经在用户词库（目前设置的是 `lua/data/user_dicts/tigress_user.dict.yaml`）文件中的会显示已在词库，不会重复添加到词库文件中

### 3.2 智能规则与限制

- **4 码自动清理**：为保持输入节奏，若未输入分隔符 `'`，输入超过 4 码时会自动清空前序编码（排除前缀如 `` ` ``）。
- **去重机制**：
  - **系统词库显示时退让**：如果虎码原生词库中已有该词，仍可以进行造词，保存到用户词库，并且显示的时候只显示用户词库中的词。
